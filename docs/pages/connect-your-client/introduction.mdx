...
<TabItem label="SAML / OIDC user">

```code
$ tsh login --proxy=<Var name="teleport.example.com"/> --user=<Var name="user"/> --auth=your-idp-connector
Enter password for Teleport user alice:
Tap any security key
> Profile URL:        https://teleport.example.com:443
  Logged in as:       alice
...
access to resources as Telport Connect, and additional access to to Request and
...
Desktop Acesss sessions. See [Using Teleport Connect](./teleport-connect.mdx).
